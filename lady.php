...
class Lady{
  ...
  # ---------------------------------------------
  static public function parse($source, $flags = 0){
    $code = $noVar = null;
    $dump = $comment = array();

    # get shrink method from flags
    $shrink = self::PRESERVE;
    if ($flags & self::STRIP){
      $shrink = self::STRIP;}
    if ($flags & self::COMPRESS){
      $shrink = self::COMPRESS;}
    ...
    foreach ($tokens as $n => $token){
      ...
      elseif ($name == T_STRING
          ...
          && $tokens[$n - 1][1] != '->'
          && ($tokens[$n - 1][1] != '.'
             || preg_match(self::REGEX_CLASS, $tokens[$n - 2][1]))
          && preg_match(self::REGEX_VARIABLE, $string)
  ...
  # ---------------------------------------------
  static public function cacheFile($file, $cacheFile, $flags = 0){
  ...
  # ---------------------------------------------
  static public function parseFile($file, $cacheFile = null, $flags = 0){
    if ($cacheFile == null){
  ...
  # ---------------------------------------------
  static public function includeFile($file, $cacheFile = null, $flags = 0){
    if ($cacheFile == null){
  ...
  # testFile
  # ---------------------------------------------
  static public function testFile($file, $flags = 0){
