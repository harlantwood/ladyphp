...
# This is a minified version of Starship Debug and Nette Debugger.
# http://github.com/unu/starship
# http://github.com/nette/nette

?><?php 
namespace
Star{class
Debug{static$timeStart=null;static$enabled=false;static$dumps=[];static$errors=[];static$logLevel=1;static$barX='right';static$logo=null;static
function
enable(){if(!self::$enabled&&in_array($_SERVER['REMOTE_ADDR'],['127.0.0.1','::1'])){if(self::$logo===null){self::$logo=(class_exists('Star\App',false))?'starship':'debug';}if(class_exists('NDebugger')){\NDebugger::enable();\NDebugger::$bar=false;}register_shutdown_function([__CLASS__,'render']);set_error_handler([__CLASS__,'errorHandler']);self::$enabled=true;}}static
function
errorHandler($severity,$message,$file,$line,$context){$types=[E_WARNING=>'Warning',E_COMPILE_WARNING=>'Warning',E_USER_WARNING=>'Warning',E_NOTICE=>'Notice',E_USER_NOTICE=>'Notice',E_STRICT=>'Strict standards',E_DEPRECATED=>'Deprecated',E_USER_DEPRECATED=>'Deprecated',];$type=isset($types[$severity])?$types[$severity]:'Unknown error';$message=$type.': '.$message.' in '.$file.': '.$line;self::$errors[$message]=isset(self::$errors[$message])?self::$errors[$message]+1:1;}static
function
memory(){return
sprintf('%.2f MB',memory_get_peak_usage()/1024/1024);}static
function
init(){if(self::$timeStart===null){self::$timeStart=microtime(true);}}static
function
totalTime(){if(isset($_SERVER['REQUEST_TIME_FLOAT'])){$time=(microtime(true)-$_SERVER['REQUEST_TIME_FLOAT'])*1000;return
sprintf('%.1f ms',$time);}elseif(self::$timeStart!==null){$time=(microtime(true)-self::$timeStart)*1000;return
sprintf('%.1f ms',$time);}}static
function
bar($variable,$title=null){self::$dumps[]=[$variable,$title];}static
function
log($variable,$priority=3){if($priority<=self::$logLevel){if(!isset(self::$dumps['log'])){self::$dumps['log']=[[],'log'];}self::$dumps['log'][0][]=$variable;}}static
function
render(){$memory=self::memory();$time=self::totalTime();$dumps=self::$dumps;$errors=self::$errors;$barX=self::$barX;$logo=self::$logo;?>
...
<ul id="starshipBar">
  <?php 
if($dumps){?>
    <div class="win" id="starshipBarDumps">
      <?php 
foreach($dumps
as$n=>$variable){list($text,$title)=$variable?>
        <h4><?php echo
...
as$key=>$line){?>
          <?php 
if(!is_string($line))$line=var_export($line,true)?>
          <pre class="row"><?php echo(is_string($key)?'<u>'.$key.'</u>: ':null).htmlspecialchars($line)?></pre>
      <?php }}?>
    </div>
  <?php }if($errors){?>
    <div class="win" id="starshipBarErrors">
      <h4>Errors</h4>
      <?php 
foreach($errors
as$message=>$n){?>
        <div class="row"><?php echo
htmlspecialchars($message).(($n>1)?' ('.$n.'x)':'')?></div>
      <?php }?>
    </div>
  <?php }if($logo){?>
    <li><b><?php echo
htmlspecialchars($logo)?></b></li>
  <?php }if($time){?>
    <li class="time" title="execution time"><?php echo
htmlspecialchars($time)?></li>
  <?php }if($memory){?>
    <li class="memory" title="peak of allocated memory"><?php echo
htmlspecialchars($memory)?></li>
  <?php }if($errors){?>
    <li id="starshipBarErrorsBtn" class="errors toggle" onclick="starshipBar.win('starshipBarErrors')">
      error
    </li>
  <?php }if($dumps){?>
    <li id="starshipBarDumpsBtn" class="dumps toggle" onclick="starshipBar.win('starshipBarDumps')">
      dump
    </li>
  <?php }?>

  <li class="close toggle" onclick="starshipBar.toggle('starshipBar')">x</li>
</ul>

<script><!--
  starshipBar = {
    wins: ['starshipBarDumps', 'starshipBarErrors'],
    toggle: function(id, to){
      el = document.getElementById(id)
      if (el){
        visible = (el.style.display != 'none')
        visible = (typeof(to) == 'boolean') ? to : !visible
        el.style.display = visible ? 'block' : 'none'
        button = document.getElementById(id + 'Btn')
        if (button)
          button.style.backgroundColor = visible ? '#ddd' : ''
      }
    },
    win: function(id){
      for (win in this.wins)
        if (this.wins[win] != id)
          this.toggle(this.wins[win], false)
      visible = (document.getElementById(id).style.display != 'none')
      this.toggle(id, !visible)
      this.save()
    },
    save: function(){
      for (win in this.wins){
        ...
        el = document.getElementById(id)
        if (el)
          document.cookie = id + '=' + (el.style.display != 'none')
      }
    },
    load: function(){
      ...
      for (win in this.wins){
        id = this.wins[win]
        for (i in cookies)
          if (cookies[i].match(RegExp(id + '=.*')))
            this.toggle(id, !!cookies[i].match(/.*=true/))
      }
    },
  }
  starshipBar.load()
...
<style>
  #starshipBar *{
    ...
    -ms-user-select: display;
  }
  #starshipBar{
    display: table;
    position: fixed;
    bottom: 0;
    <?php echo($barX=='left')?$barX:'right'?>: 1em;
    height: 2em;
    margin: 0 10px;
    padding-left: 8px;
  ...
  #starshipBar .win{
    display: none;
    position: absolute;
    <?php echo($barX=='left')?$barX:'right'?>: 0;
    bottom: 120%;
    min-width: 15em;
    min-height: 3em;
    max-width: 50em;
...
<!-- /Starship debug bar -->
    <?php }}Debug::init();}?><?php 
namespace{if(!function_exists('sdebug')){function
sdebug(){return
call_user_func_array('Star\Debug::enable',func_get_args());}}if(!function_exists('sbar')){function
sbar(){return
call_user_func_array('Star\Debug::bar',func_get_args());}}if(!function_exists('slog')){function
slog(){return
call_user_func_array('Star\Debug::log',func_get_args());}}}?><?php

namespace{define('NETTE_DIR',dirname(__FILE__));interface
IBarPanel{function
getTab();function
getPanel();}class
